...
/// GLM プロバイダー
///
/// Zhipu AI GLM-4V Vision API を使用したテキスト抽出
...
impl GlmProvider {
    ...
    pub fn new(config: GlmConfig) -> Result<Self> {
        if config.api_key.is_none() {
            return Err(AedError::missing_api_key("ZHIPU_API_KEY"));
        }
    ...
    pub fn with_api_key(api_key: impl Into<String>) -> Result<Self> {
        let config = GlmConfig::default().with_api_key(api_key);
        Self::new(config)
    }
    ...
    async fn call_api(&self, request: &ChatCompletionRequest) -> Result<(ChatCompletionResponse, Duration)> {
        ...
        let api_key = self
            ...
            .api_key
            .as_ref()
            .ok_or_else(|| AedError::missing_api_key("ZHIPU_API_KEY"))?;
    ...
    async fn handle_error_response(&self, response: reqwest::Response) -> AedError {
        ...
        match status {
            401 | 403 => AedError::invalid_api_key("GLM"),
...
mod tests {
    ...
    fn test_provider_creation_without_api_key() {
        ...
        let result = GlmProvider::new(config);
    ...
    fn test_data_uri() {
        ...
        let uri = GlmProvider::to_data_uri(&image);
